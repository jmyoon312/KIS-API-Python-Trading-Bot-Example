...
class TelegramView:
    ...
    def get_start_message(self, mode_name, live_vitals, latest_version):
        mode_icon = "🔥 [REAL]" if mode_name == "REAL" else "🧪 [MOCK]"
        market_phase = live_vitals.get("task_status", {}).get("current_phase", "OFF")
        ...
        status_disp = phase_map.get(market_phase, "💤 대기")
        
        dst_info = live_vitals.get("dst_info", "🌞 서머타임(17:30)")
        is_summer = "서머" in dst_info or "17:30" in dst_info
        season_icon = "🌞" if is_summer else "❄️"
        
        # 🕒 스케줄 동적 구성
        if is_summer:
            sched_txt = "▫️ 08:30 : 📝 자동 동기화\n▫️ 17:00 : 🌅 프리마켓 감시\n▫️ 22:30 : 🔥 정규장 매매 시작\n▫️ 05:00 : 🌙 애프터 수거\n▫️ 06:00 : 🧹 자정 및 분석"
        else:
            sched_txt = "▫️ 10:45 : 📝 자동 동기화\n▫️ 18:00 : 🌅 프리마켓 감시\n▫️ 23:30 : 🔥 정규장 매매 시작\n▫️ 06:00 : 🌙 애프터 수거\n▫️ 07:00 : 🧹 자정 및 분석"

        total_asset = live_vitals.get("cash", 0) + live_vitals.get("holdings_value", 0)
        
        return (
            f"🌌 <b>Infinity Quant Hub {mode_icon}</b>\n"
            f"⚡ <b>오퍼레이셔널 상황실 가동 중</b>\n\n"
            f"📊 <b>[ 실시간 현황 ]</b>\n"
            f"▫️ <b>상태</b>: {status_disp} (OS Live)\n"
            f"▫️ <b>자산</b>: ${total_asset:,.0f}\n"
            f"▫️ <b>버전</b>: {latest_version}\n\n"
            f"🕒 <b>[ 운영 스케줄 ({season_icon}) ]</b>\n"
            f"{sched_txt}\n\n"
            f"🛠️ <b>[ 상황실 주요 명령어 ]</b>\n"
            f"▶️ <b>/sync</b> : 📜 통합 지시서(TrueSync)\n"
            f"▶️ <b>/record</b> : 📊 일자별 통합 장부\n"
            f"▶️ <b>/seed</b> : 💵 시드머니(할당 비중) 관리\n"
            f"▶️ <b>/ticker</b> : 🔄 운용 종목 변경\n"
            f"▶️ <b>/settlement</b> : ⚙️ 전체 설정 확인\n"
            f"▶️ <b>/mode</b> : 🏎️ 매매 모드(가속/일반)\n"
            f"▶️ <b>/log</b> : 📋 최근 운영 기록(아카이브)\n"
            f"▶️ <b>/history</b> : 🏆 명예의 전당(졸업)\n\n"
            f"💡 <b>아래 버튼을 터치하여 상황실을 제어하세요.</b>"
        )

    def get_situation_room_markup(self):
        keyboard = [
            [
                InlineKeyboardButton("📊 실시간 현황(TrueSync)", callback_data="DASH:SYNC"),
            ],
            [
                InlineKeyboardButton("📋 최근 운영 기록", callback_data="DASH:LOG"),
                InlineKeyboardButton("📈 성과 리포트", callback_data="DASH:ANALYTICS")
            ],
            [
                InlineKeyboardButton("⚙️ 시스템 설정", callback_data="DASH:SETTINGS"),
                InlineKeyboardButton("🔐 안전 통제(Reset)", callback_data="RESET:MENU")
            ],
            [
                InlineKeyboardButton("📖 전략 가이드(백서)", url="https://github.com/jmyoon312/Infinity-Hub")
            ]
        ]
        return InlineKeyboardMarkup(keyboard)

    def get_log_display_message(self, events):
        if not events:
            return "📭 기록된 운영 내역이 없습니다."
            
        msg = "📋 <b>[ 최근 시스템 운영 기록 ]</b>\n\n"
        for ev in reversed(events[-10:]):
            icon = "✅" if ev['status'] == "SUCCESS" else "⚠️" if ev['status'] == "WARNING" else "❌"
            msg += f"{icon} <code>{ev['time']}</code> <b>{ev['task']}</b>\n"
            msg += f"└ {ev['msg']}\n\n"
    ...
    def get_reset_menu(self, active_tickers):
        msg = (
            "🛠️ <b>[ 시스템 안전 통제실 ]</b>\n"
            "⚠️ 주의: 매매 잠금(당일 주문 완료 상태)을 강제 해제할 종목을 선택하세요."
        )
        keyboard = []
        
        for t in active_tickers:
            keyboard.append([InlineKeyboardButton(f"🔓 [{t}] 금일 매매 잠금 해제", callback_data=f"RESET:LOCK:{t}")])
    ...
    def create_sync_report(self, status_text, dst_text, cash, rp_amount, ticker_data, is_trade_active):
        ...
        for t_info in ticker_data:
            ...
            if t_info['t_val'] > (t_info['split'] * 1.1):
                ...
                body_msg += f"🔎 현재 T값(<b>{t_info['t_val']:.4f}T</b>)이 설정된 분할수(<b>{int(t_info.get('dynamic_split', t_info['split']))}회</b>)를 초과했습니다!\n"
                body_msg += f"💡 <b>원인 역산 추정:</b> 수동 매수로 수량이 급증했거나, '/seed' 시드머니 설정이 대폭 축소되었습니다.\n"
                body_msg += f"🛡️ <b>가동 조치:</b> 마이너스 호가 차단용 절대 하한선($0.01) 방어막 가동 중!\n\n"

            turbo_off_t = t_info['split'] * 0.75
            turbo_on_t = t_info['split'] * 0.5
            
            # 🌟 [V22 스마트 어드바이저] 가속 모드 ON/OFF 자동 권장
            if t_info['t_val'] >= turbo_off_t and t_info['turbo_txt'] == "ON":
                body_msg += f"🤖 <b>[💡 V22 스마트 어드바이스: {t} 가속 OFF 권장]</b>\n"
                body_msg += f"📉 <i>대폭락장(3단기어) 구간 진입! 시드 소진 방어를 위해 /mode 를 터치하여 '일반'으로 전환하세요.</i>\n\n"
            elif t_info['t_val'] < turbo_on_t and t_info['turbo_txt'] == "OFF":
                body_msg += f"🤖 <b>[💡 V22 스마트 어드바이스: {t} 가속 ON 권장]</b>\n"
                body_msg += f"📈 <i>안정권 회복! 휩소 반등장 수익 극대화를 위해 /mode 를 터치하여 '가속'을 켜주세요.</i>\n\n"
            ...
            if is_rev:
                ...
                body_msg += f"📈 진행: <b>{t_info['t_val']:.4f}T / {int(t_info.get('dynamic_split', t_info['split']))}분할</b>\n"
            else:
                bdg_txt = f"1일 권장 진입액 ${t_info['one_portion']:,.0f}" if v_mode in ["V14", "V17"] else f"1회 ${t_info['one_portion']:,.0f}"
                body_msg += f"{main_icon} <b>[{t}] {v_mode_display}</b>\n"
                body_msg += f"📈 동적기어: <b>{t_info['t_val']:.4f}T / {int(t_info.get('dynamic_split', t_info['split']))}회 분할장전</b>\n"
            ...
            # 스나이퍼 방어선 코멘트 제거됨 (V22에서 동적 분할이 모든 폭락 방어를 흡수)
