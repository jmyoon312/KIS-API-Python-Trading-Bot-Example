...
class TelegramController:
    def __init__(self, cfg, broker, strategy, mode_name, tx_lock=None):
        """
        🚀 [V23.1 Dual-Core] 단일 엔진 전용 텔레그램 컨트롤러
        """
        self.cfg = cfg
        self.broker = broker
        self.strategy = strategy
        self.mode_name = mode_name.upper() # "REAL" or "MOCK"
    ...
    def _is_admin(self, update: Update):
        ...
        if self.admin_id is None:
            print(f"⚠️ [{self.mode_name}] 보안 경고: ADMIN_CHAT_ID가 설정되지 않아 접근을 차단했습니다.")
            return False
    ...
    def update_live_vitals(self, vitals):
        """
        💓 [Heartbeat] 엔진으로부터 실시간 바이탈 데이터를 수신하여 저장
        """
        self.live_vitals = vitals

    def _calculate_budget_allocation(self, cash, tickers, cfg):
        ...
        for tx in sorted_tickers:
            # 🌟 [V22 패치] 100% 동적분할 시스템이 도입되어, 이제 TelegramBot 차원의 
            # 예산 공유 풀(Cash Pool) 분배를 생략하고 각 종목별 Seed 한도 내에서 최대한 배정합니다.
            allocated[tx] = cfg.get_seed(tx)
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # 🚀 [V24] 상황실 대시보드 데이터 취합
        live_vitals = self.cfg.get_analytics_data() 
        latest_version = self.cfg.get_latest_version()
        
        msg = self.view.get_start_message(self.mode_name, live_vitals, latest_version)
        markup = self.view.get_situation_room_markup()
        
        await update.message.reply_text(msg, reply_markup=markup, parse_mode='HTML')

    async def cmd_log(self, update, context):
        if not self._is_admin(update): return
        events = self.cfg.get_recent_events(limit=10)
        msg = self.view.get_log_display_message(events)
    ...
    async def cmd_sync(self, update, context):
        ...
        # 🚨 [V22.2 TrueSync] 데이터 소스 단일화 (환경별 live_status.json 참조)
        LIVE_FILE = self.cfg._get_file_path("LIVE_STATUS")
        TRIGGER_FILE = self.cfg._get_file_path("REFRESH_TRIGGER")
        
        # 콜백 쿼리인 경우 처리
        if hasattr(update, "callback_query") and update.callback_query:
            target = update.callback_query.message
        else:
            target = update.message if hasattr(update, "message") else None

        if target:
            status_msg = await target.reply_text(f"🔄 <b>[{self.mode_name}] 시장 분석 및 지시서 동기화 중...</b>", parse_mode='HTML')
        else:
            return
        ...
        # 봇 엔진이 파일을 갱신할 때까지 잠시 대기 (최대 5초)
        for _ in range(10):
            await asyncio.sleep(0.5)
            if not os.path.exists(TRIGGER_FILE): break

        if not os.path.exists(LIVE_FILE):
            await status_msg.edit_text("❌ 동기화 데이터 파일을 찾을 수 없습니다. 잠시 후 다시 시도해주세요.")
            return
        ...
        try:
            ...
            # 텔레그램 View 형식에 맞게 데이터 변환
            ticker_data_list = []
            for t, info in data['tickers'].items():
                # 텔레그램 View에서 기대하는 필드 구성
                t_info = {
                    'ticker': t,
                    'version': info['version'],
                    't_val': info['t_val'],
                    'split': info['split'],
                    'dynamic_split': info.get('dynamic_split', info['split']),
                    'curr': info['current_price'],
                    'avg': info['avg_price'],
                    'qty': info['qty'],
                    'profit_amt': info['profit_amt'],
                    'profit_pct': info['profit_pct'],
                    'turbo_txt': info['turbo_mode'],
                    'target': info.get('target_profit', 0.0), # 필요한 경우 config 참조 가능
                    'star_pct': 0.0, 
                    'seed': info['seed'],
                    'one_portion': info['seed'] / info['dynamic_split'] if info['dynamic_split'] > 0 else 0,
                    'plan': {'orders': info['orders'], 'process_status': info['process_status']},
                    'is_locked': info['is_locked'],
                    'is_reverse': info.get('process_status') == "🩸리버스(긴급수혈)",
                    'star_price': 0.0,
                    'escrow': info.get('escrow_cash', 0.0),
                    'day_high': info.get('day_high', 0.0),
                    'day_low': info.get('day_low', 0.0),
                    'prev_close': info.get('prev_close', 0.0)
                }

                # 주문 목록에서 별값(star_price) 및 타겟 수익률 등 추출
                for o in info['orders']:
                    if "별값" in o['desc']: t_info['star_price'] = o['price']
                    if "익절" in o['desc'] and t_info['target'] == 0:
                        try:
                            # 익절: $65.23 (10.0%) -> 10.0 추출 시도
                            if "(" in o['desc'] and "%" in o['desc']:
                                p_str = o['desc'].split("(")[1].split("%")[0]
                                t_info['target'] = float(p_str)
                        except: pass

                ticker_data_list.append(t_info)

            # RP 추천 금액 계산 (잉여금의 95%)
            surplus = data['available_cash']
            ...
            final_msg, markup = self.view.create_sync_report(
                ...
                rp_amount, 
                ticker_data_list, 
                data['is_trade_active']
            )
            await status_msg.delete() # 기존 "동기화 중..." 메시지 삭제
            await context.bot.send_message(update.effective_chat.id, final_msg, reply_markup=markup, parse_mode='HTML')
            
        except Exception as e:
            await status_msg.edit_text(f"❌ 데이터 해석 중 오류 발생: {str(e)}")
    ...
    async def cmd_record(self, update, context):
        ...
        status_msg = await context.bot.send_message(chat_id, f"🛡️ <b>[{self.mode_name}] 장부 무결성 검증 중...</b>", parse_mode='HTML')
        ...
        if success_tickers: 
            ...
            await status_msg.edit_text(f"✅ <b>[{self.mode_name}] 동기화 완료</b>", parse_mode='HTML')
                ...
                if actual_qty == 0:
                    if ledger_qty > 0:
                        ...
                        try:
                            ...
                            tot_eval = 0.0
                            if fresh_holdings:
                                tot_eval += int(fresh_holdings.get("TQQQ", {}).get("qty", 0)) * (t_price if t_price > 0 else float(fresh_holdings.get("TQQQ", {}).get("avg", 0)))
                                tot_eval += int(fresh_holdings.get("SOXL", {}).get("qty", 0)) * (s_price if s_price > 0 else float(fresh_holdings.get("SOXL", {}).get("avg", 0)))
                            
                            total_asset = cash + tot_eval
                            # 2. 계단식 복리를 위해 해당 종목 매수비율만큼으로 시드 벌크업 수행
                            rebalanced_seed = self.cfg.rebalance_seed_on_graduation(ticker, total_asset)
                        except Exception as e:
                            logging.error(f"졸업 리밸런싱 중 오류: {e}")
                            rebalanced_seed = self.cfg.get_seed(ticker)
                            
                        msg = f"🎉 <b>[{ticker} 졸업 확인!]</b>\n장부를 명예의 전당에 저장하고 새 사이클을 준비합니다."
                        msg += f"\n🔄 <b>[계단식 복리 시스템 가동]</b>\n현재 계좌 총자산 기준 할당 비율에 맞춰 {ticker}의 시드가 <b>${rebalanced_seed:,.0f}</b> 로 영구 갱신(벌크업) 되었습니다!"
    ...
    async def cmd_seed(self, update, context):
        ...
        # 🌟 [V22 패치] 시드 메뉴에서 계좌 총자산 및 포트폴리오 비중 기준 권장 시드액 계산
        cash, holdings = self.broker.get_account_balance()
        if holdings is None:
            total_asset = 0
            msg = "💵 <b>[ 종목별 시드머니 관리 ]</b>\n(⚠️ API 오류로 현재 자산을 가져오지 못했습니다)\n\n"
        else:
            try:
                t_price = await asyncio.to_thread(self.broker.get_current_price, "TQQQ", is_market_closed=True)
                s_price = await asyncio.to_thread(self.broker.get_current_price, "SOXL", is_market_closed=True)
                ...
                tot_eval = 0.0
                if holdings:
                    tot_eval += int(holdings.get("TQQQ", {}).get("qty", 0)) * (t_price if t_price > 0 else float(holdings.get("TQQQ", {}).get("avg", 0)))
                    tot_eval += int(holdings.get("SOXL", {}).get("qty", 0)) * (s_price if s_price > 0 else float(holdings.get("SOXL", {}).get("avg", 0)))
                
                total_asset = cash + tot_eval
            except Exception as e:
                total_asset = 0
                
            msg = f"💵 <b>[ 종목별 시드머니 관리 ]</b>\n"
            msg += f"🏦 계좌 총자산(추정): <b>${total_asset:,.0f}</b>\n\n"

        try:
            portfolio_ratios = self.cfg._load_json(self.cfg.FILES["PORTFOLIO_RATIO"], {"TQQQ": 55, "SOXL": 45})
        except:
            portfolio_ratios = {"TQQQ": 55, "SOXL": 45}

        keyboard = []
        for t in self.cfg.get_active_tickers():
            current_seed = self.cfg.get_seed(t)
            ratio = portfolio_ratios.get(t, 0)
            recommended = total_asset * (ratio / 100.0) if total_asset > 0 else 0
            
            msg += f"💎 <b>{t}</b>: ${current_seed:,.0f}\n"
            if recommended > 0:
                msg += f"💡 <i>{ratio}% 비중 적정 투자금: ${recommended:,.0f}</i>\n\n"
            else:
                msg += "\n"
        ...
        if total_asset > 0:
            keyboard.append([InlineKeyboardButton("⚖️ 계좌 총자산 연동: 적정 시드 원클릭 셋업", callback_data="SEED:REBALANCE:ALL")])
    ...
    async def cmd_settlement(self, update, context):
        ...
        for t in self.cfg.get_active_tickers():
            ...
            p_ratio = portfolio_ratios.get(t, 0)
            msg += f"{icon} <b>{t} ({ver_display} 모드)</b>\n▫️ 기본 분할: <b>{int(self.cfg.get_split_count(t))}회 (동적변속 탑재)</b>\n▫️ 목표 익절: <b>{self.cfg.get_target_profit(t)}%</b>\n▫️ 포트폴리오 비중: <b>{p_ratio}%</b> (새출발 시)\n▫️ 자동복리율(보존): <b>{self.cfg.get_compound_rate(t)}%</b>\n\n"
            
            row1 = [
                InlineKeyboardButton(f"⚙️ {t} 분할", callback_data=f"INPUT:SPLIT:{t}"), 
                InlineKeyboardButton(f"🎯 {t} 익절목표", callback_data=f"INPUT:TARGET:{t}"),
                InlineKeyboardButton(f"💸 {t} 복리비율", callback_data=f"INPUT:COMPOUND:{t}")
            ]
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "DASH":
            if sub == "SYNC": await self.cmd_sync(query, context)
            elif sub == "LOG":
                events = self.cfg.get_recent_events(limit=10)
                msg = self.view.get_log_display_message(events)
                # 뒤로가기 버튼 추가 (옵션)
                keyboard = [[InlineKeyboardButton("🔙 상황실 홈으로", callback_data="DASH:HOME")]]
                await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
            elif sub == "ANALYTICS":
                vitals = self.cfg.get_analytics_data()
                total = vitals.get("cash", 0) + vitals.get("holdings_value", 0)
                msg = (
                    f"📈 <b>[{self.mode_name}] 성과 분석 요약</b>\n\n"
                    f"▫️ <b>총 평가 자산</b>: ${total:,.2f}\n"
                    f"▫️ <b>현금 잔고</b>: ${vitals.get('cash', 0):,.2f}\n"
                    f"▫️ <b>주식 가치</b>: ${vitals.get('holdings_value', 0):,.2f}\n\n"
                    f"💡 상세 리포트 및 자산 추이 그래프는 웹 대시보드(Visual Studio 코드 상단 버튼)에서 더욱 자세히 확인하실 수 있습니다."
                )
                keyboard = [[InlineKeyboardButton("🔙 상황실 홈으로", callback_data="DASH:HOME")]]
                await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
            elif sub == "SIMULATOR":
                await context.bot.send_message(query.message.chat_id, "🧪 Simulation Lab: 최근 16년 백테스트 분석 결과 요약 준비 중...")
            elif sub == "SETTINGS":
                await self.cmd_settlement(query, context)
            elif sub == "HOME":
                live_vitals = self.cfg.get_analytics_data()
                msg = self.view.get_start_message(self.mode_name, live_vitals, self.cfg.get_latest_version())
                markup = self.view.get_situation_room_markup()
            ...
            if sub == "REBALANCE":
                await query.edit_message_text("⚖️ <b>계좌 자산 연동 자동 리밸런싱 중...</b>", parse_mode='HTML')
                cash, holdings = self.broker.get_account_balance()
                if holdings is None:
                    await query.edit_message_text("❌ API 오류로 현재 자산을 가져오지 못했습니다.", parse_mode='HTML')
                    return
                try:
                    t_price = await asyncio.to_thread(self.broker.get_current_price, "TQQQ", is_market_closed=True)
                    s_price = await asyncio.to_thread(self.broker.get_current_price, "SOXL", is_market_closed=True)
                    ...
                    msg = f"⚖️ <b>[ 원클릭 시드 리밸런싱 완료! ]</b>\n\n"
                    msg += f"🏦 실시간 계좌 총자산: <b>${total_asset:,.0f}</b>\n\n"
                    
                    for t in self.cfg.get_active_tickers():
                        rebalanced = self.cfg.rebalance_seed_on_graduation(t, total_asset)
                        msg += f"💎 <b>{t}</b> 👉 <b>${rebalanced:,.0f}</b> (저장 반영)\n"
                        
                    msg += "\n✅ TQQQ/SOXL 포트폴리오 철학에 맞게 개별 운용 시드가 완벽히 재조정되었습니다!"
                    await query.edit_message_text(msg, parse_mode='HTML')
                except Exception as e:
                    await query.edit_message_text(f"❌ 리밸런싱 중 오류 발생: {e}", parse_mode='HTML')
            else:
                ticker = data[2]
                self.user_states[update.effective_chat.id] = f"SEED_{sub}_{ticker}"
                await context.bot.send_message(update.effective_chat.id, f"💵 [{ticker}] 금액 입력 (숫자만, $단위):")
        elif action == "INPUT":
            ticker = data[2]
